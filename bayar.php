...
$cek=mysql_num_rows(mysql_query("SELECT * FROM pesan_packages WHERE id_pesan_packages='".$_GET[id_pesan]."' AND username='".$USERNAME."' AND token='".$toktok."'"));
...
?>
	...
	<table cellpadding="7px" cellspacing="0"width="100%" height="200px" bgcolor="#ffffff">
	 <tr>
		 <td align="left" valign="middle">
				<font color="#ff0000"><strong>Token pembelian yang Anda masukkan salah</strong></font><br><br>
				<a href="javascript:history.back(-1)" class="link_kecil">Kembali ke halaman sebelumnya</a>
			</td>
		</tr>
	</table>
...
else
{
 ...
 $masuk="UPDATE pesan_packages SET status='1', tanggal_konfirmasi='".$tanggal_update."', nama_bank='".$_POST[nama_bank]."', nomor_rekening='".$_POST[nomor_rekening]."', atas_nama='".$_POST[atas_nama]."', tranfer_ke='".$_POST[kategori]."' WHERE id_pesan_packages='".$_GET[id_pesan]."'";
	$res_masuk=mysql_query($masuk);
	if($res_masuk)
	{
		$masuk_log=mysql_query("INSERT INTO log (username, tanggal, ket) VALUES ('".$USERNAME."', '".$tanggal_update."', 'KONFIRMASI PEMBAYARAN')");
		if($masuk_log)
		{
		?>
			<table cellpadding="7px" cellspacing="0"width="100%" height="200px" bgcolor="#ffffff">
				<tr>
					<td align="left" valign="middle">
						...
						Terima kasih atas konfirmasi pembayaran yang Anda lakukan. Administrator kami akan melakukan verifikasi terhadap pembayaran yang Anda lakukan.<br>
