...
<table cellpadding="5px" cellspacing="0" border="0" width="100%">
	<tr>
	 <td align="left" valign="middle" class="judul_barang">
		 <big><strong><?php echo $ambil[nama_barang]; ?> | SHOW PRICE</strong></big>
		</td>
	</tr>
	<tr>
 ...
 <tr>
	...
	<tr>
	 <td align="center" valign="middle">
		 <?php 
				$aa=rand("1000", "5000");
				$aaa=$_GET[id]."".$lebar.$aa;
			 $matauang=mysql_fetch_array(mysql_query("SELECT * FROM matauang WHERE idmatauang='".$ambil[idmatauang]."'"));
			?>
		 <h2>Special Price: <?php echo $matauang[matauang]; ?> <?php echo number_format($ambil[harga_baru], 0, ',', '.'); ?>,-</h2>
			<br><br>
			<?php
			$booo=mysql_num_rows(mysql_query("SELECT * FROM booking WHERE id_product='".$_GET[id]."' AND (status='0' OR status='1')"));
			if(!empty($USERNAME)&&$booo<3)
			{
			?>
			<img style="cursor:pointer" onClick="document.location='booking.php?id=<?php echo $_GET[id]; ?>&token=<?php echo $aaa; ?>&harga_baru=<?php echo $ambil[harga_baru]; ?>'" src="images/booking.png" align="absmiddle" />
		 <?php
			}
			else
			{
   ?>
			<strong><i>Full Booked</i></strong>
			<?php			
			}
			?>
			<br /><br />
			<?php
			 $book=mysql_query("SELECT * FROM booking WHERE id_product='".$_GET[id]."' AND (status='0' OR status='1')");
				$book2=mysql_num_rows($book);
				if($book2)
				{
				 $n=1;
				 ?>
					 <strong>List Booking: </strong><br>
					<?php
					while($book3=mysql_fetch_array($book))
					{
					 echo $n.". ". $book3[username]." ( ".$book3[tanggal]." )<br>";  
					 $n++;
					}
				}
			?>
